    ...
    for resume in myResumes:
        for skill in resume["skills"]:
            ...
            if (skillName not in skillsAvailable and (skillName not in ignoreList)):
                ...
                mySkillSupply[skillsAvailable.index(skillName)][1] = mySkillSupply[skillsAvailable.index(skillName)][
                                                                         1] + 1;
...
def generate_final_dataset(jobs, skillsAvailable, mySkillSupply):
    ...
    for jobKey in jobs:
        ...
        for jobSkill in jobSkills:
            mySkillSupply[skillsAvailable.index(str.lower(jobSkill))][2] = \
            mySkillSupply[skillsAvailable.index(str.lower(jobSkill))][2] + 1;
...
def final_cleaning():
    ...
    ignoreList = ['collection', 'operation', 'jenkins', 'training', "dynamics", 'javascript', "marketing",
                  'recruitment', 'publishing', 'quantitative'];
